...
def scrape_single_game_data(game_info, season, week):
    ...
    game_data = {'starters': [], 'snap_counts': []}
    ...
    if away_starters_div:
        ...
        if away_starters_table:
            ...
            if away_starters:
                game_data['starters'].extend(away_starters)
    ...
    if home_starters_div:
        ...
        if home_starters_table:
            ...
            if home_starters:
                game_data['starters'].extend(home_starters)
    ...
    if away_snap_div:
        ...
        if away_snap_table:
            ...
            if away_snaps:
                game_data['snap_counts'].extend(away_snaps)
    ...
    if home_snap_div:
        ...
        if home_snap_table:
            ...
            if home_snaps:
                game_data['snap_counts'].extend(home_snaps)
    ...
    print(f"✓ Game complete - Starters: {len(game_data['starters'])}, Snap Counts: {len(game_data['snap_counts'])}")
    time.sleep(3)  # Be respectful to the server
    
    return game_data

@task(cache_policy=NO_CACHE)
def save_data_to_files(all_games_data, season, week, week_path):
    """Save combined starters and snap counts data to Excel files"""
    ...
    for game_data in all_games_data:
        if game_data['starters']:
            all_starters_data.extend(game_data['starters'])
        if game_data['snap_counts']:
            all_snap_counts_data.extend(game_data['snap_counts'])
...
def scrape_starters_and_snap_counts_flow(season, week):
    """Flow to scrape starters and snap counts for a specific season and week"""
    ...
    for game_info in games_info:
        game_data = scrape_single_game_data(game_info, season, week)
        all_games_data.append(game_data)
    
    # Save combined data
    save_data_to_files(all_games_data, season, week, week_path)
